...
class Amoli
{
     ...
     * @return 搜索结果
     */
    public function searchLocalFiles($dir, $keyword)
    {
        $result = [];
        $this->searchLocalFilesRecursive($dir, $keyword, $result, $dir);
        empty($result) ? $result[] = ['type' => 'null', 'name' => '未找到匹配的文件', 'size' => '', 'time' => '', 'path' => ''] : '';
        return $result;
    }

    private function searchLocalFilesRecursive($dir, $keyword, &$result, $baseDir)
    {
        ...
        foreach ($list as $file) {
            ...
            $file_location = $dir . DIRECTORY_SEPARATOR . $this->getEncoding($file, true);
            if ($encode) {
            ...
            if (is_dir($file_location)) {
                $this->searchLocalFilesRecursive($file_location, $keyword, $result, $baseDir);
            } else {
                if (stripos($file, $keyword) !== false) {
                    $relativePath = str_replace($baseDir, '', dirname($file_location));
     ...
     * @return 搜索结果
     */
    public function searchOssFiles($bucket, $endpoint, $accessKeyId, $accessKeySecret, $dir, $keyword)
    {
        try {
            $ossClient = new OssClient($accessKeyId, $accessKeySecret, $endpoint);
            $options = array('prefix' => $dir, 'max-keys' => '1000');
            ...
            if (!empty($objectList)) {
                foreach ($objectList as $objectInfo) {
                    $fileName = str_replace($dir, '', $objectInfo->getKey());
                    if ($fileName && stripos($fileName, $keyword) !== false) {
                        $pathParts = explode('/', $fileName);
                        $name = array_pop($pathParts);
                        $path = implode('/', $pathParts);
                        
                        $result[] = [
                            'type' => $this->getStamp($name),
                            'name' => $name,
                            'size' => $this->getFilesize($objectInfo->getSize()),
                            'time' => date("Y-m-d H:i", strtotime($objectInfo->getLastModified())),
                            'path' => $path ? '/' . $path : ''
                        ];
                    }
                }
            ...
            $result = [['type' => 'null', 'name' => '搜索出错：' . $e->getMessage(), 'size' => '', 'time' => '', 'path' => '']];
        }
        return $result;
    }
     ...
     * @return 搜索结果
     */
    public function searchCosFiles($bucket, $region, $secretId, $secretKey, $dir, $keyword)
    {
        $cosClient = new Qcloud\Cos\Client(array('region' => $region, 'schema' => 'https', 'credentials' => array('secretId'  => $secretId, 'secretKey' => $secretKey)));
        try {
            $list = $cosClient->listObjects(array('Bucket' => $bucket, 'Prefix' => $dir, 'MaxKeys' => 1000));
            $objectList = $list['Contents'];
            $result = [];

            if (!empty($objectList)) {
                foreach ($objectList as $objectInfo) {
                    $fileName = str_replace($dir, '', $objectInfo['Key']);
                    if ($fileName && stripos($fileName, $keyword) !== false) {
                        $pathParts = explode('/', $fileName);
                        $name = array_pop($pathParts);
                        $path = implode('/', $pathParts);
                        ...
                        $result[] = [
                            'type' => $this->getStamp($name),
                            'name' => $name,
                            'size' => $this->getFilesize($objectInfo['Size']),
                            'time' => date("Y-m-d H:i", strtotime($objectInfo['LastModified'])),
                            'path' => $path ? '/' . $path : ''
                        ];
                    }
                }
     ...
     * @return 统计数据
     */
    public function getLocalStorageStats($dir)
    {
        $stats = [
            'total_size' => 0,
            'total_files' => 0,
        ...
        $this->getLocalStorageStatsRecursive($dir, $stats);
        
        // 格式化数据
        $stats['total_size_formatted'] = $this->getFilesize($stats['total_size']);
        arsort($stats['file_types']);
        
        return $stats;
    }

    private function getLocalStorageStatsRecursive($dir, &$stats)
    {
        if (!is_dir($dir)) return;
        $list = scandir($dir);
        ...
        foreach ($list as $file) {
            ...
            if (is_dir($file_location)) {
                $this->getLocalStorageStatsRecursive($file_location, $stats);
            } else {
                $filesize = filesize($file_location);
                $stats['total_size'] += $filesize;
                $stats['total_files']++;
                
                $ext = $this->getStamp($file);
     ...
     * @return 统计数据
     */
    public function getOssStorageStats($bucket, $endpoint, $accessKeyId, $accessKeySecret, $dir)
    {
        try {
            $ossClient = new OssClient($accessKeyId, $accessKeySecret, $endpoint);
            $options = array('prefix' => $dir, 'max-keys' => '1000');
     ...
     * @return 统计数据
     */
    public function getCosStorageStats($bucket, $region, $secretId, $secretKey, $dir)
    {
        $cosClient = new Qcloud\Cos\Client(array('region' => $region, 'schema' => 'https', 'credentials' => array('secretId'  => $secretId, 'secretKey' => $secretKey)));
        try {
            $list = $cosClient->listObjects(array('Bucket' => $bucket, 'Prefix' => $dir, 'MaxKeys' => 1000));
